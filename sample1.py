...
def sample(eval_args):
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            ...
            # Decode the surfaces
            surf_ncs = surf_vae(surfZ.unflatten(-1, torch.Size([16, 3])).flatten(0, 1).permute(0, 2, 1).unflatten(-1,
                                                                                                                  torch.Size(
                                                                                                                      [
                                                                                                                          4,
                                                                                                                          4])))
            surf_ncs = surf_ncs.permute(0, 2, 3, 1).unflatten(0, torch.Size(
                [batch_size, num_surfaces])).detach().cpu().numpy()

            # Decode the edges
            edge_ncs = edge_vae(edge_z.unflatten(-1, torch.Size([4, 3])).reshape(-1, 4, 3).permute(0, 2, 1))
            edge_ncs = edge_ncs.permute(0, 2, 1).reshape(batch_size, num_surfaces, num_edges, 32,
                                                         3).detach().cpu().numpy()
    ...
    for batch_idx in range(batch_size):
        ...
        try:
            unique_faces, unique_edges, FaceEdgeAdj, EdgeVertexAdj = detect_shared_edge(unique_vertices,
                                                                                        new_vertex_dict, edge_z_cad,
                                                                                        surf_z_cad, z_threshold,
                                                                                        edge_mask_cad)
        except Exception as e:
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                surf_ncs_cad = surf_vae(
                    torch.FloatTensor(unique_faces).cuda().unflatten(-1, torch.Size([16, 3])).permute(0, 2,
                                                                                                      1).unflatten(-1,
                                                                                                                   torch.Size(
                                                                                                                       [
                                                                                                                           4,
                ...
                edge_ncs_cad = edge_vae(
                    torch.FloatTensor(unique_edges).cuda().unflatten(-1, torch.Size([4, 3])).permute(0, 2, 1))
                edge_ncs_cad = edge_ncs_cad.permute(0, 2, 1).detach().cpu().numpy()

        #### 3-3: Joint Optimize ###
        num_edge = len(edge_ncs_cad)
        num_surf = len(surf_ncs_cad)
        surf_wcs, edge_wcs = joint_optimize(surf_ncs_cad, edge_ncs_cad, surf_pos_cad, unique_vertices, EdgeVertexAdj,
                                            FaceEdgeAdj, num_edge, num_surf)
        ...
        write_stl_file(solid, f'{save_folder}/{random_string}_{batch_idx}.stl', linear_deflection=0.001,
